...
{
    class ChangeMonitorThumbnailTest
    {
        IWebDriver driver;
        ...
        [Test]
        public void ChangeMonitorThumbnail()
        {
            ...
            //save current thumbnail in a varible for later assertion
            var thumbnailSrc = driver.FindElement(By.Id("thumbnail")).GetProperty("src");
            ...
            var changeThumbnailButton = driver.FindElement(By.Id("setThumbnail"));
            changeThumbnailButton.Click();

            Thread.Sleep(3000);
            var result = context.Monitors.Find(monitorToBeEditedId);
            context.Entry(result).Reload();
            Assert.IsFalse(thumbnailSrc.Contains(result.Thumbnail));
        }
